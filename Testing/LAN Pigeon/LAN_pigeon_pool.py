...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.sidebar_button_1 = customtkinter.CTkButton(self.sidebar_frame, text="Start Scan", command=lambda: threading.Thread (target=self.scan_call, args=(self.start_entry.get(), self.end_entry.get(), self.progress_var)).start())
    ...
    def scan_process(self, ip):
        # Execute the three functions in sequence for the given IP address
        available = tool.connect(ip)

        if available == 'Yes':
            ping_time = tool.ping(ip)
            hostname = tool.hostname(ip)
            mac_address = tool.mac_address(ip)

            self.print_debug(f"[{currentTime}] Processed IP Address: {ip} , Processed Ping: {ping_time}"
                             f" , Processed Hostname: {hostname}\n"
                             f"[{currentTime}] Processed MAC: {mac_address} ,"
                             f"Processed Connectivity: {available}\n")
            self.results_treeview.insert('', 'end', values=(ip, hostname, mac_address, ping_time, available))

    def scan_call(self, start_ip, end_ip, progress_var):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for ip in ips:
                executor.submit(self.scan_process, ip)
